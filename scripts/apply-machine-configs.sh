...
echo "======================================"
echo "IMPORTANT: Omni Workflow"
echo "======================================"
echo ""
echo "⚠️  Omni requires machines to be part of a cluster to apply patches."
echo ""
echo "The correct workflow is:"
echo ""
echo "1. Create the cluster WITH machine assignments:"
echo "   omnictl cluster template sync -f ${CONFIG_DIR}/cluster-template.yaml"
echo ""
echo "   This will:"
echo "   - Create the cluster"
echo "   - Assign machines to control plane and worker roles"
echo "   - Apply all patches (hostnames, labels, configs)"
echo ""
echo "2. OR use Omni UI:"
echo "   - Go to Omni UI → Clusters → Create Cluster"
echo "   - Select machines by their UUIDs"
echo "   - Machines will get hostnames and labels from the cluster template"
echo ""
echo "3. After cluster is created, verify patches:"
echo "   omnictl get configpatches"
echo "   omnictl get machines -o json | jq '.items[] | {uuid: .metadata.id, hostname: .spec.network.hostname}'"
echo ""
echo "======================================"
echo "Quick Start"
echo "======================================"
echo ""
echo "To create the cluster with all patches applied:"
echo ""
echo "  omnictl cluster template sync -f ${CONFIG_DIR}/cluster-template.yaml"
echo ""
echo "This single command will:"
echo "  ✓ Create the cluster"
echo "  ✓ Assign machines to roles (control-plane, worker, gpu-worker)"
echo "  ✓ Apply hostnames (talos-control-1, talos-worker-1, etc.)"
echo "  ✓ Apply node labels (management-ip, node-role, zone)"
echo "  ✓ Apply role-specific configs (GPU support, Longhorn, etc.)"
echo ""

echo "======================================"
echo "Alternative: Manual Machine Selection"
echo "======================================"
echo ""
echo "If you prefer to select machines manually in Omni UI:"
echo ""
echo "Machine UUIDs by role:"
echo ""
echo "Control Planes:"
echo "${MATCHED_MACHINES}" | jq -r '.[] | select(.role == "control-plane") | "  \(.hostname): \(.omni_uuid)"'
echo ""
if [[ ${WORKER_COUNT} -gt 0 ]]; then
    echo "Workers:"
    echo "${MATCHED_MACHINES}" | jq -r '.[] | select(.role == "worker") | "  \(.hostname): \(.omni_uuid)"'
    echo ""
fi
if [[ ${GPU_COUNT} -gt 0 ]]; then
    echo "GPU Workers:"
    echo "${MATCHED_MACHINES}" | jq -r '.[] | select(.role == "gpu-worker") | "  \(.hostname): \(.omni_uuid)"'
    echo ""
fi
echo ""
...
echo "======================================"
echo ""
echo "1. Review the cluster template:"
echo "   cat ${CONFIG_DIR}/cluster-template.yaml"
echo ""
echo "2. Create the cluster (recommended):"
echo "   omnictl cluster template sync -f ${CONFIG_DIR}/cluster-template.yaml"
echo ""
echo "3. Monitor cluster creation:"
echo "   omnictl get machines --watch"
echo "   omnictl get clusters"
echo ""
...
echo "  - Machines must be part of a cluster for patches to apply"
echo "  - Create cluster first, then patches will apply automatically"
echo ""
echo "If you see 'Error: expected 4 elements node':"
echo "  - This is normal when trying to apply Machine resources directly"
echo "  - Use 'omnictl cluster template sync' instead"
echo ""
echo "To see all available machines:"
echo "  omnictl get machines -o json | jq '.items[] | {uuid: .metadata.id, connected: .spec.connected}'"
echo ""
